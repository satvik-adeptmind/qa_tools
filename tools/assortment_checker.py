...
def render_assortment_checker():
    """Renders the Assortment Checker (Word Checker) tool."""

    st.header("🔎 Assortment Checker")
    st.markdown(
        "Use this tool to check if search results contain **all** the required 'word' groups."
    )

    # --- Sidebar Config ---
    with st.sidebar:
        st.markdown("---")
        st.subheader("⚙️ Assortment Checker Config")
        shop_id = st.text_input("Shop ID", value="", key="ac_shop_id")
        environment = st.selectbox("Environment", ["prod", "staging"], index=0, key="ac_env")
        search_result_size = st.number_input(
            "Search Result Size", min_value=1, max_value=1000, value=500, key="ac_result_size"
        )

        st.markdown("---")
        st.subheader("Check Group Management")
        if "ac_check_groups_state" not in st.session_state:
            st.session_state.ac_check_groups_state = [""]
            st.session_state.ac_match_types_state = ["Text Contains"]

        def add_check_group():
            st.session_state.ac_check_groups_state.append("")
            st.session_state.ac_match_types_state.append("Text Contains")

        def remove_check_group(index):
            st.session_state.ac_check_groups_state.pop(index)
            st.session_state.ac_match_types_state.pop(index)

        def reset_check_groups():
            st.session_state.ac_check_groups_state = [""]
            st.session_state.ac_match_types_state = ["Text Contains"]

        st.button("Add another check group", on_click=add_check_group, use_container_width=True, key="ac_add_grp")
        st.button("Reset to 1 check group", on_click=reset_check_groups, use_container_width=True, key="ac_reset_grp")

    # --- Main Area: Check Groups ---
    st.subheader("Required Concepts")
    st.markdown("A product is relevant **only if it contains a match from EACH group below**.")

    for i in range(len(st.session_state.ac_check_groups_state)):
        st.markdown(f"**Check Group {i+1}**")
        col1, col2, col3 = st.columns([4, 2, 1])
        with col1:
            st.session_state.ac_check_groups_state[i] = st.text_input(
                "Comma-separated variations",
                value=st.session_state.ac_check_groups_state[i],
                key=f"ac_check_group_input_{i}",
                label_visibility="collapsed",
                placeholder=f"e.g., hdr10+, hdr 10+, hdr 10plus",
            )
        with col2:
            st.session_state.ac_match_types_state[i] = st.radio(
                "Match Type",
                options=["Text Contains", "Text Equals"],
                key=f"ac_match_type_{i}",
                horizontal=True,
                label_visibility="collapsed",
            )
        with col3:
            if len(st.session_state.ac_check_groups_state) > 1:
                st.button(
                    f"🗑️ Delete",
                    key=f"ac_del_{i}",
                    on_click=remove_check_group,
                    args=(i,),
    ...
    if submitted:
        check_group_inputs = st.session_state.ac_check_groups_state
        ...
        if not all([shop_id, search_keyword_input] + [inp.strip() for inp in check_group_inputs]):
            ...
            check_groups = [
                ...
                for group_str in check_group_inputs
                if group_str.strip()
            ]
            match_types = st.session_state.ac_match_types_state
            ...
            if not check_groups:
                ...
                with st.spinner(f"Analyzing top {search_result_size} results for '{search_keyword}'..."):
                    analysis_result = run_analysis(
                        shop_id.strip(), environment, search_keyword, check_groups, match_types, search_result_size
                    )
                ...
                if analysis_result["status"] == "error":
                    ...
                    col1.metric(
                        "Relevance Score",
                        f"{len(relevant_list)} / {total}",
                        help="Products containing a match from ALL check groups.",
                    )
                        ...
                        if irrelevant_list:
                            ...
                            with st.expander("Show Failure Analysis"):
                                ...
                                for group_idx, count in sorted(failure_summary.items()):
                                    ...
                                    summary_data.append(
                                        {"Missing Concept Group": group_name, "Number of Products Failed": count}
                                    )
                                if summary_data:
                            ...
                            for item in irrelevant_list:
                                failed_reasons = [
                                    f"'{check_groups[idx][0]}...'" for idx in item["failed_indices"]
                                ]
